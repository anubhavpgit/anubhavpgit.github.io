...
\section{Experience}
  \resumeSubHeadingListStart
    ...
    {\textbf{AI Research Engineer Intern}}{ May 2025 | August 2025}
    \resumeItemListStart
      \resumeItem
        {Built full-stack video-sharing platform (Next.js 15, Vercel, Supabase) winning World Builder Residency
   at Edge Esmeralda. Engineered distributed video pipeline with Redis job queue (900+ concurrent jobs), custom batch
  processing reducing DB writes 70\%, and Web3 authentication via World ID MiniKit SDK. Achieved 3x faster loads
  through multi-tier caching, adaptive polling, and SWR optimization.}
    \resumeItem
        {Deployed scalable microservices architecture (40+ APIs) on Vercel with PostgreSQL/Redis infrastructure achieving
  99.9\% uptime. Implemented social features (follows, likes, reconciliation), rate limiting, distributed locks,
  circuit breakers, and automated background services for cleanup/monitoring with graceful fallbacks and connection
  pooling.}
